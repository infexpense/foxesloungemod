...
public class CreaksPortalRenderer<T extends CreaksPortalBlockEntity> implements BlockEntityRenderer<T> {


    public CreaksPortalRenderer(BlockEntityRendererProvider.Context context){
    }

    @Override
    public void render(T pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
        Matrix4f matrix4f = pPoseStack.last().pose();
        this.renderCube(pBlockEntity, matrix4f, pBuffer.getBuffer(RenderType.endGateway()));
    }
